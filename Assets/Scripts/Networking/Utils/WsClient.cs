...
namespace Networking.Utils
{
    public class WsClient
    {
        ...
        {
            Debug.Log("Disonnecting from: " + serverUri);
            await ws.CloseAsync(WebSocketCloseStatus.NormalClosure, "status", CancellationToken.None);
            Debug.Log("Close status: " + ws.State);
        }
        ...
        private async void RunSend()
        {
            ...
            while (true)
            {
                while (!sendQueue.IsCompleted)
                {
                    ...
                    await ws.SendAsync(msg, WebSocketMessageType.Text, true /* is last part of message */,
